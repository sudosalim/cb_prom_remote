...
func (c *Config) Validate() error {
	...
	switch c.Storage.TimeSeriesType {
	case "regular", "irregular":
		// valid
	default:
		return fmt.Errorf("storage timeseries_type must be 'regular' or 'irregular', got %q", c.Storage.TimeSeriesType)
	}
	if c.Storage.TimeSeriesType == "regular" {
		if c.Storage.RegularSampleInterval <= 0 {
			return fmt.Errorf("storage regular_sample_interval must be positive when timeseries_type is regular")
		}
		if c.Storage.TimeSeriesInterval < c.Storage.RegularSampleInterval {
			return fmt.Errorf("storage timeseries_interval must be >= regular_sample_interval when timeseries_type is regular")
		}
	}
